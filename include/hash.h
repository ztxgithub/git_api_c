...
{
	int key_len;  //key的长度
	int value_len;  //value的长度
	int malloc_value_size;  //分配value空间的大小,这个只有当HashArray->is_malloc_value为true时才有效,而且通过跟value比较看是否申请新的内存

#ifdef HASH_STORE_HASH_CODE
	unsigned int hash_code;  //保存的hashcode
#endif

	char *value;  //存储value的空间
	struct tagHashData *next;
	char key[0];  //实现key的动态增长,HashData->key 代表下一个数据结构的开始地址
} HashData;
...
{
	HashData **buckets;
	HashFunc hash_func;  //hash函数
	int item_count;  //item 数目,所有桶中的节点总个数
	unsigned int *capacity;  //总桶数, 根据自定义要申请的总桶数得到合适的桶个数(是一个素数数组prime_array)
	double load_factor;  //平衡因子,填充因子
	int64_t max_bytes;
	int64_t bytes_used;      // 等于 sizeof(HashData *) * (*capacity)
	bool is_malloc_capacity;  //是否分配空间,一般false
	bool is_malloc_value;  //是否给value分配了空间,如果为true,则HashArray结构体内自己管理value的内容,value值不够存,动态申请新的内容给value存
							//如果为false,则HashData->value 内存需要有hash.h以外显示申请好传入
	unsigned int lock_count;
	pthread_mutex_t *locks;  //可以准确到对每一个桶进行分别互斥,不同的桶相互之间不影响
} HashArray;
 ...
 *         key: the key to find
 *         key_len: length of th key 
 * return user data(value), return NULL when the key not exist
*/
...
/**
 * hash walk (iterator) 对hash表中所有的节点都进行遍历操作
 * parameters:
 *         pHash: the hash table
