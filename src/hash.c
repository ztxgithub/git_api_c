...
/*主要根据(*pHash->capacity)数申请(HashData *)类型的内存空间
 * 对pHash->bytes_used进行赋值
 * */
...
/*重新调整HashArray中桶的个数，
 * 如果是增加桶的个数,那么新申请new_capacity大小的空间存放HashData *,
 * 再将原来的各个桶的数据节点移动到新的hash表中*/
...
/*增加HashArray->capacity　桶的个数,
 * 如果HashArray->is_malloc_capacity为true(自己申请内存保存HashArray->capacity)
 * 开始用数组prime_array里的值,并且将is_malloc_capacity置为false
 *
 * 如果HashArray->is_malloc_capacity为false(HashArray->capacity用的是prime_array数组的内容)
 * pHash->capacity++
 *
 * _rehash1函数
 * */
 ...
 * pHash:指向hash表的指针
 * key：sectionName
 * key_len:sectionName的长度
 *
 * 主要是通过哈希函数计算出哈希地址,得到对应的桶的地址,
 * 再根据桶的地址,匹配key相同的,返回对应的value
 * */
void *hash_find(HashArray *pHash, const void *key, const int key_len)
{
	...
	hash_code = pHash->hash_func(key, key_len);  //计算hashcode,哈希地址
	ppBucket = pHash->buckets + (hash_code % (*pHash->capacity));  //通过这种策略，确定hash的表的入口条目

	HASH_LOCK(pHash, ppBucket - pHash->buckets)
	hash_data = _chain_find_entry(ppBucket, key, key_len, hash_code);  //从入口条目开始，遍历链表(同义词桶)
	HASH_UNLOCK(pHash, ppBucket - pHash->buckets)
...
/*对(key,value)进行哈希插入,如果已经存在则修改value值,返回0
 * 在这个工程中HashArray->max_bytes == 0,HashArray->is_malloc_value 为false
 * 如果当前的状态pHash->item_count/(double)*pHash->capacity大于填充因子,那么增加hash表的大小
 * */
		...
		void *value, const int value_len, const bool needLock)
{
	...
	hash_data = *ppBucket;  //取到特定的桶的入口地址
	while (hash_data != NULL)
	{
		if (key_len == hash_data->key_len && \
			memcmp(key, hash_data->key, key_len) == 0)
